...
class ChatflowDesignContext(BaseModel):
    ...
    mf_node_ids: set = Field(..., description="Node IDs from main_flows only, knowledge is not inlcuded.")
    mf_starting_node_ids: set = Field(..., description="Starting node IDs from main_flows only, knowledge is not inlcuded.")
...
class ChatFlowConfig(BaseModel):
    ...
    def from_paths(
            ...
            knowledge_main_flow_path: str | Path,
        ...
        # TODO: 1. Load agent data and prepare agent configuration
        ...
        agent_config = AgentConfig(
            ...
            # 设置-大模型匹配
            use_llm=bool(agent_data.get("use_llm")),
        ...
        # TODO: 2. Load knowledge and knowledge main flows, and prepare knowledge context
        ...
        with open(knowledge_main_flow_path, 'r', encoding='utf-8') as f:
            knowledge_main_flow = json.load(f)

        if not isinstance(knowledge_main_flow, list):
            e_m = "知识库流程数据应为列表"
            logger_chatflow.error(e_m)
            raise TypeError(e_m)
        ...
        # TODO: 3. Load chatflow design and prepare chatflow context
        with open(chatflow_design_path, 'r', encoding='utf-8') as f:
            chatflow_design = json.load(f)

        if not isinstance(chatflow_design, list):
            e_m = "流程设计文件应为列表"
            logger_chatflow.error(e_m)
            raise TypeError(e_m)
        ...
        for flow in chatflow_design:
            ...
            if not isinstance(starting_node_id, str) or not starting_node_id:
                ...
                logger_chatflow.error(e_m)
                raise TypeError(e_m)
            if not isinstance(base_nodes, list): # It can be empty list as sometimes there are no base nodes in a main flow
        ...
        # TODO 4. Load globals and prepare global config context
        with open(global_path, 'r', encoding='utf-8') as f:
            global_configs_raw = json.load(f)

        if not isinstance(global_configs_raw, list):
            e_m = "全局配置数据必须为列表"
            logger_chatflow.error(e_m)
            raise TypeError(e_m)
        ...
        # TODO: 6. Load vector_db_collection
    ...
    def from_files(
        ...
        # TODO: 5. Prepare intentions

        # TODO: 6. Manage vector_db_collection
        vector_db_collection_lookup = {v["node_id"]: v["db_collection_name"] for v in vector_db_collection}
